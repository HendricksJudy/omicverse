...
def test_agent_initialization_with_skills_no_crash(capsys):
    ...
    with patch("omicverse.utils.smart_agent.PANTHEON_INSTALLED", True):
        ...
        # NOTE: This function is imported from skill_registry in smart_agent
        with patch("omicverse.utils.skill_registry.build_multi_path_skill_registry", return_value=mock_registry):
            # Mock SkillRouter
            with patch("omicverse.utils.skill_registry.SkillRouter"):
                # Mock ModelConfig to avoid validation issues
                with patch("omicverse.utils.model_config.ModelConfig") as mock_model_config:
...
def test_agent_skill_count_with_multiple_sources(capsys):
    ...
    with patch("omicverse.utils.smart_agent.PANTHEON_INSTALLED", True):
        ...
        builtin_skill.summary_text = "A builtin skill for testing"  # Add summary_text
        ...
        user_skill.summary_text = "A user skill for testing"  # Add summary_text
        ...
        # Create a mock SkillRouter instance
        mock_router_instance = MagicMock()

        with patch("omicverse.utils.skill_registry.build_multi_path_skill_registry", return_value=mock_registry):
            with patch("omicverse.utils.skill_registry.SkillRouter", return_value=mock_router_instance):
                with patch("omicverse.utils.model_config.ModelConfig") as mock_model_config:
                    mock_model_config.validate_model_setup.return_value = (True, "Model ready")
                    mock_model_config.get_provider_from_model.return_value = "openai"
