...
Usage:
    ...
    # Run analysis tasks with natural language
    result = agent.run("quality control with nUMI>500, mito<0.2", adata)

    # Create skills from documentation
    skill_info = ov.Agent.seeker("https://example.com/docs", name="Example Skill")
"""
...
class Agent:
    ...
    def __init__(self, model: str = "gpt-4o-mini", api_key: Optional[str] = None, provider: Optional[str] = None, endpoint: Optional[str] = None, **opts):
        """
        ...
        provider : str, optional
            Provider name (e.g., "openai", "anthropic"). If not provided, inferred from model
        endpoint : str, optional
            Custom API endpoint. If not provided, will use default for the provider
        **opts
            Additional options passed to the agent (reserved for future use)
        """
        ...
        # Normalize model ID to handle aliases and variations
        # This provides backward compatibility for unprefixed model IDs
    ...
    def _initialize_skill_registry(self) -> None:
        """
        Load skills from both package installation and user's working directory.

        Skills are discovered from two locations (in priority order):
        1. Package root: omicverse/.claude/skills (built-in skills)
        2. Current working directory: ./.claude/skills (user-created skills)

        User-created skills override built-in skills if they have the same slug.
        """
        ...
        if registry and registry.skills:
            ...
            # Count skills by source for user feedback
            ...
            skill_msg = f"   🧭 Loaded {total} skills"
            if builtin_count > 0 and user_count > 0:
                skill_msg += f" ({builtin_count} built-in + {user_count} user-created)"
            elif builtin_count > 0:
                skill_msg += f" ({builtin_count} built-in)"
            elif user_count > 0:
                skill_msg += f" ({user_count} user-created)"
            print(skill_msg)
        else:
        ...
        return json.dumps(
            {
                ...
                "instructions": definition.prompt_instructions(provider=self.provider),
...
'''
    ...
    def _format_skill_guidance(self, matches: List[SkillMatch]) -> str:
        """Format skill instructions for prompt injection with provider-specific optimization."""

        if not matches:
            return ""
        blocks = []
        for match in matches:
            # Use provider-specific formatting for optimal LLM comprehension
    ...
    def list_available_skills(self) -> List[str]:
        """
        List all available project skills discovered by the agent.

        Returns a list of skill names (slugs) that have been loaded from
        the .claude/skills directory. This is useful for debugging and
        verifying that skills created by ov.Agent.seeker() have been
        discovered.

        Returns
        -------
        list[str]
            List of skill names (slugs) discovered by the agent.
            Returns empty list if no skills are available.

        Examples
        --------
        >>> agent = ov.Agent(model="gpt-4o-mini", api_key="...")
        >>> # Agent prints: "🧭 Loaded 2 project skills from .claude/skills"
        >>> skills = agent.list_available_skills()
        >>> print(skills)
        ['example-tool', 'my-library-docs']

        >>> # Verify a specific skill was discovered
        >>> if 'my-library-docs' in agent.list_available_skills():
        ...     print("Skill discovered successfully!")
        """
        if not self.skill_registry or not self.skill_registry.skills:
            return []
        return list(self.skill_registry.skills.keys())
    ...
    def seeker(
        ...
        *,
        ...
        """
        Create a Claude Agent skill from documentation link(s) for quick prototyping.

        This method creates skills by scraping documentation websites or combining
        multiple sources. It's ideal for rapid skill creation from existing documentation.
        ...
        >>> result = ov.Agent.seeker("https://example.com/docs", name="Example Skill")
        >>> print(result['slug'])  # 'example-skill'
        >>>
        >>> # Multiple sources with packaging
        >>> result = ov.Agent.seeker(
        ...     ["https://docs.site-a.com/", "https://docs.site-b.com/guide"],
        ...     name="Multi Source",
        ...     package=True
        ... )
        >>> print(result['zip'])  # Path to .zip file
        """
        return _seeker_impl(
            links,
            name=name,
    ...
    @staticmethod
    def supported_models(show_all: bool = False) -> str:
        """
        ...
        --------
        >>> import omicverse as ov
        >>> print(ov.Agent.supported_models())
        >>> print(ov.Agent.supported_models(show_all=True))
        """
...
class OmicVerseAgent(Agent):
    """
    Deprecated alias for Agent class.

    .. deprecated::
        Use :class:`ov.Agent` instead. This alias will be removed in a future version.

    This class is provided for backward compatibility only. All new code should use
    the canonical :class:`ov.Agent` class directly.
    """
    def __init__(self, *args, **kwargs):
        warnings.warn(
            "OmicVerseAgent is deprecated; use ov.Agent instead",
            DeprecationWarning,
            stacklevel=2
        )
